...
.func <- function (param, value) {
  ...
  grid <- as_tibble(expand.grid(i = i, j = i, k = 1:nrow(value$target_operator_id)))
...
.func <- function (param, value) {
  ...
  load_actual_config <- function (q_source, q_sink, irrep_row) {
  ...
  actual_loaded <- mapply(load_actual_config, grid$source_relative_momenta, grid$sink_relative_momenta, grid$irrep_row, SIMPLIFY = FALSE, USE.NAMES = FALSE)
...
.func <- function (param, value) {
  list(summary = value$loaded_long)
}

loaded_summary <- pv_call(.func, loaded_long)
```
...
```{r}
plots <- loaded_summary %>%
  group_by(total_momentum_str, irrep) %>%
  do(plot = ggplot(
    .,
    aes(x = time,
        y = abs(correlator),
        color = kind,
        shape = factor(irrep_row))) +
    geom_point(position = position_dodge(width = 0.3)) +
    scale_y_log10() +
    facet_grid(source_relative_momenta ~ sink_relative_momenta) +
    labs(title = "Comparison between Martin's and Markus's projection code",
         subtitle = sprintf('P = %s, irrep = %s, cnfg = %d',
