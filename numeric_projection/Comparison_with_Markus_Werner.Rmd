...
.func <- function (param, value) {
  ...
  actual_loaded <- mapply(load_actual_config, grid$source_relative_momenta, grid$sink_relative_momenta, grid$irrep_row, grid$irrep_row, SIMPLIFY = FALSE, USE.NAMES = FALSE)
...
plots <- loaded_summary %>%
  ...
  do(plot = ggplot(
    ...
    aes(x = time,
        y = abs(correlator),
        color = kind,
        shape = interaction(irrep_row, sprintf("%d", irrep_col)))) +
    geom_point(position = position_dodge(width = 0.3)) +
    scale_y_log10() +
    facet_grid(source_relative_momenta ~ sink_relative_momenta) +
    labs(title = "Comparison between Martin's and Markus's projection code",
