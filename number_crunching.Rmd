...
```{r}
all_prescriptions <- jsonlite::read_json('gevp-rho--1-11-A2.js')
```
...
```{r}
needed_names <- unique(unlist(lapplyn(all_prescriptions, function (rule) rule$datasetname, 5)))
```
...
resolved <- lapplyn(all_prescriptions, resolve)
```
